...
func main() {
	...
	fmt.Println("---My Own Tests---")

	news := ScrapeHackerNews(50)
	fmt.Println("Printing NewsSlice...")
	...
	for i, emailAddress := range GetEmails() {
		fmt.Println(i)
	...
	gdpCanada, _ := GetCountryGDP("Canada")
	fmt.Println(gdpCanada)
	gdpColombia, _ := GetCountryGDP("Colombia")
	fmt.Println(gdpColombia)
	gdpBahrain, _ := GetCountryGDP("Bahrain")
	fmt.Println(gdpBahrain)
	_, err = GetCountryGDP("Korea")
	fmt.Println(err)
	gdpGermany, _ := GetCountryGDP("Germany")
	fmt.Println(gdpGermany)
}
...
func ScrapeHackerNews(n int) NewsSlice {
	...
	if err != nil {
		log.Fatalf("Unable to create document: %v\n", err)
	}

	res := make(NewsSlice, 0)

	pointsDoc := doc.Find("span.score")         // <span>X points</span>
	titlesAndURLsDoc := doc.Find("a.storylink") // <a href=URL>TITLE</a>
	usernamesDoc := doc.Find("a.hnuser")        // <a ...>USERNAME</a>

	for i := 0; i < n; i++ {
		pointsText := pointsDoc.Eq(i).Text()
		points, err := strconv.Atoi(pointsText[:strings.Index(pointsText, " ")])
		if err != nil {
			log.Fatalf("Ill formatted points: %v\n", err)
		}

		title := titlesAndURLsDoc.Eq(i).Text()
		url, ok := titlesAndURLsDoc.Eq(i).Attr("href")
		if !ok {
			log.Fatalf("Non-existent URL for title: %s\n", title)
		}

		username := usernamesDoc.Eq(i).Text()
...
func GetEmails() []string {
	...
	emailAddressesDoc := doc.Find("tr a")
	mailTo := "mailto:"

	for i := range emailAddressesDoc.Nodes {
		emailAddress, ok := emailAddressesDoc.Eq(i).Attr("href")
		if ok && strings.Index(emailAddress, mailTo) == 0 {
			res = append(res, emailAddress[len(mailTo):])
		}
	}
...
func GetCountryGDP(country string) (int, error) {
	ch := make(chan CountryData, 1)
	timeout := time.After(time.Second * 10)

	go scrapeCountryGDP(ch,
		"https://www.cis.upenn.edu/~cis193/scraping/9828772efc2bd314a277c8880695dea2.html")

	for {
		select {
		case res := <-ch:
			if res.Country == country {
				gdp, err := strconv.Atoi(strings.Replace(res.GDP, ",", "", -1))
				if err != nil {
					log.Fatalf("Ill formatted GDP: %v\n", err)
				}
				return gdp, nil
			}
		case <-timeout:
			return -1, errors.New("timeout occurred")
		}
	}
}

func scrapeCountryGDP(ch chan CountryData, url string) {
	...
	ch <- CountryData{
		Country: doc.Find("h3.country").Eq(0).Text(),
		GDP:     doc.Find("h3.gdp").Eq(0).Text(),
	}

	otherCountries := doc.Find("a")
	for i := range otherCountries.Nodes {
		linkedURL, ok := otherCountries.Eq(i).Attr("href")
		if ok {
			go scrapeCountryGDP(ch, linkedURL)
		}
	}
}
